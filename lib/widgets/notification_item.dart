...
import 'package:forgecubes/widgets/issue_icon.dart';
...
class NotificationItem extends StatefulWidget {
  ...
  NotificationItem({Key? key, required this.payload, required this.markAsRead})
    : super(key: key);
...
class _NotificationItemState extends State<NotificationItem> {
  ...
  void _markAsRead() async {
    if (payload.unread! && !loading) {
      ...
      try {
        await context.read<AuthModel>().ghClient.activity.markThreadRead(
          payload.id!,
        );
        widget.markAsRead();
      } finally {
