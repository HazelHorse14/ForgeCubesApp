...
import 'package:forgecubes/widgets/loading.dart';
...
class _GlSearchScreenState extends State<GlSearchScreen> {
  ...
  Future<void> _query() async {
    ...
    try {
      ...
      final projects = await auth.fetchGitlabWithPage(
        '/search?scope=projects&search=$keyword',
      );
      final users = await auth.fetchGitlabWithPage(
        '/search?scope=users&search=$keyword',
      );
  ...
  Widget _buildItem(_p) {
    if (_activeTab == 0) {
      ...
      return RepositoryItem.gl(payload: p, note: 'Updated $updatedAt');
    } else {
