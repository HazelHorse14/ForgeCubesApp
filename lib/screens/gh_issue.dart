...
import 'package:forgecubes/widgets/avatar.dart';
import 'package:forgecubes/widgets/link.dart';
import 'package:forgecubes/widgets/timeline_item.dart';
...
class GhIssueScreen extends StatelessWidget {
  ...
  Future<GIssueData_repository> _queryIssue(
    BuildContext context, {
    String? cursor,
  }) async {
    ...
    OperationResponse<GIssueData, GIssueVars?> res = await context
        .read<AuthModel>()
        .gqlClient
        .request(req)
        .first;
    ...
    return LongListStatefulScaffold<GIssueData_repository, dynamic>(
      ...
      trailingBuilder: (p) {
        if (p.issueOrPullRequest!.G__typename == 'Issue') {
                ...
                ActionItem(
                  ...
                  onTap: (_) async {
                    await context.read<AuthModel>().ghClient.issues.edit(
                      ...
                      number,
                      github.IssueRequest(state: d.closed ? 'open' : 'closed'),
                    );
                  },
                ),
          ...
          return ActionButton(
            title: 'Actions',
            items: [...ActionItem.getUrlActions(d.url)],
          );
        }
      },
      headerBuilder: (p) {
