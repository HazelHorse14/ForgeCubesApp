...
import 'package:forgecubes/utils/nil_store.dart';
// import 'package:in_app_review/in_app_review.dart';
import 'package:universal_io/io.dart';
import 'package:forgecubes/models/bitbucket.dart';
import 'package:forgecubes/models/gitea.dart';
import 'package:forgecubes/models/gitee.dart';
...
class AuthModel with ChangeNotifier {
  ...
  Future<void> _onSchemeDetected(Uri? uri) async {
    ...
    final res = await http.post(
      Uri.parse('https://forgecubes-oauth.vercel.app/api/token'),
      headers: {
...
''', token);
  ...
  Future<DataWithPage> fetchGiteaWithPage(
    ...
    return DataWithPage(
      ...
      total:
          int.tryParse(res.headers['x-total-count'] ?? '') ??
          TOTAL_COUNT_FALLBACK,
    );
  }
  ...
  Future fetchBbJson(
    ...
    final res = await fetchBb(p, isPost: isPost, body: body);
  ...
  Future loginToGitee(String token) async {
    ...
    try {
      ...
      notifyListeners();
      final res = await http.get(
        Uri.parse('https://gitee.com/api/v5/user'),
        headers: {'Authorization': 'token $token'},
      );
      final info = json.decode(res.body);
      if (info['message'] != null) {
  ...
  void redirectToGithubOauth([publicOnly = false]) {
    ...
    launchUrl(
      'https://github.com/login/oauth/authorize?client_id=$clientId&redirect_uri=forgecubes://login&scope=$scope&state=$_oauthState',
    );
  }
